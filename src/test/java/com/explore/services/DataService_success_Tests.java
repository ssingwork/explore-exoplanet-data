...
public class DataService_success_Tests {
    ...
    public void setUp() {
        ...
        File outfile =  new File(this.getClass().getResource("/sample_data.json").getFile());;
...
//    @PrepareForTest({ DownloadUtils.class })
    @Test
    public void findPlanets_Success() throws Exception{
        List<Planet>  planetOrbitingHottestStar = dataManager.findPlanetOrbitingHottestStar();
        Assert.assertTrue("check orbit found ",planetOrbitingHottestStar !=null);
        Assert.assertTrue("check 1 hottest start found ",planetOrbitingHottestStar.size() ==1);
    }
        ...
        for(int i=0;i<data.size();i++){
            if(data.get(i).getYear()==2004){
                if(data.get(i).getSize().equals("small")) {
                    Assert.assertEquals(data.get(i).getPlanetCount(),2);
                }else if(data.get(i).getSize().equals("medium")) {
                    Assert.assertEquals(data.get(i).getPlanetCount(),5);
